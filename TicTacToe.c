...
// insert(player, pos, board) inserts player's mark in the pos-th square of board
...
// print_board(board) prints the game in progress
void print_board(const char board[]){
	for(int row = 0; row < 3; ++row){
		printf("|");
		for(int col = 0; col < 3; ++col){
